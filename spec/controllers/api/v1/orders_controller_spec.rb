...
RSpec.describe Api::V1::OrdersController, type: :controller do
  describe 'POST #create' do
    ...
    before do
      ...
      # Stub the initialization and method call separately
      stripe_payment_instance = instance_double(OrdersService::StripePayment)
      allow(OrdersService::StripePayment).to receive(:new).with(email:, items: permitted_items).and_return(stripe_payment_instance)
    ...
    context 'when the request is successful' do
      ...
      it 'creates a Stripe checkout session' do
        ...
        expect(OrdersService::StripePayment).to have_received(:new).with(email:, items: permitted_items)
        expect(OrdersService::StripePayment.new(email:, items: permitted_items)).to have_received(:create_checkout_session)
      end
