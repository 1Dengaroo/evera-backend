...
RSpec.describe OrderDeliveredJob, type: :job do
  ...
  before do
    allow(NotificationsService::Order::OrderDelivered).to receive(:send)
  end
  ...
  describe '#perform' do
    it 'calls the service' do
      ...
      expect(NotificationsService::Order::OrderDelivered).to have_received(:send).with(to: email, order:)
    end
  end
end
